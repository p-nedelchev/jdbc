...
public class ConcreteTripRepository implements TripRepository {
    ...
    public ConcreteTripRepository(Connection connection){
    ...
    public List<Trip> searchTrip(String egn) {
        List<Trip> foundTrips = new LinkedList<>();
        String search = "SELECT * FROM people WHERE name LIKE '" + egn + "%'";
        ...
        try {
            ...
            while (result.next()) {
                String newEgn = result.getString("egn");
                LocalDate newDateOfArrival = result.getDate("arrival_date").toLocalDate();
                LocalDate newDateOfDeparture = result.getDate("departure_date").toLocalDate();
                String newCity = result.getString("city");
                Trip trip = new Trip(newEgn, newDateOfArrival, newDateOfDeparture, newCity);
                foundTrips.add(trip);
            }
        } catch (SQLException e) {
            e.printStackTrace();
